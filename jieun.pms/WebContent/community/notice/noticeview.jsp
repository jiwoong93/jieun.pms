...
<body>
	<form name="input_form" action="./noticeupdate.jsp">
						...
						<td>
							<div class="board_head">
								<table width="604" border="1" style="border-collapse: collapse;">
									...
									<tr>
										<td>작성일</td>
										<td width="180"><%=post.getNoticeDate()%><input type="hidden" name="noticeDate" value="<%=post.getNoticeDate()%>"></td>
										<td width="117">조회수</td>
										<td width="512"><%=post.getNoticeView()%><input type="hidden" name="noticeView" value="<%=post.getNoticeView()%>"></td>
									</tr>
								</table>
							</div>
							<table class="board_table2" border="1" align="center">
								<tr>
									<td height="202" colspan="4">
										<% if(post.getNoticeContents() != null && !post.getNoticeContents().equals("")){
										%> <%=post.getNoticeContents() %>
										<%}else{
										%><%=""%>
										<%}
